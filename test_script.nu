...
def execute [window_size data_size iterations duration = 5] {
    mpirun --wdir . --hostfile hostfile -mca pml ucx --mca btl ^vader,tcp,openib,uct -x UCX_NET_DEVICES=mlx5_1:1 ./target/release/openshmem-benchmark -w $window_size -s $data_size -d $duration -n $iterations
}
